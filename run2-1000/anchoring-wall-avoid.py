...
while t < t_centromere:# and t_total < tmax:
    ...
    if [(x[t]-1),y[t],z[t]] in coodr or x[t]-1 < 1 or x[t]-1 < -box_side/2 or x[t]-1 < -box_side/2:
        left = False
    else:
        left = True
...
while t < tmax:# and t_total < tmax:
    ...
    if t == t_centromere:
        safe_t = np.nan
        # lfd: avoid steps further than x = 1 in negative-x direction.
        if [(x[0]-1),y[0],z[0]] in coodr or x[0]-1 < 1 or x[0]-1 < -box_side/2 or x[0]-1 < -box_side/2:
            left = False
        else:
            left = True
        if [(x[0]+1),y[0],z[0]] in coodr or x[0]+1 > box_side/2 :
            right = False
        else:
            right = True
        if [(x[0]),(y[0]+1),z[0]] in coodr or y[0]+1 > box_side/2:
            front = False
        else:
            front = True
        if [(x[0]),(y[0]-1),z[0]] in coodr or y[0]-1 < -box_side/2:
            ...
            behind = True
        if [(x[0]),(y[0]),(z[0]+1)] in coodr or z[0]+1 > box_side/2:
            up = False
        else:
            up = True
        if [(x[0]),(y[0]),(z[0]-1)] in coodr or z[0]-1 < -box_side/2:
...
with open('data5','w') as fout:
    ...
    fout.write("LAMMPS data file\n\n%d atoms\n%d bonds\n%d angles\n\n2 atom types\n1 bond types\n1 angle types\n\n0.0 %f xlo xhi\n0.0 %f ylo yhi\n0.0 %f zlo zhi\n\nMasses\n\n1 %f\n2 %f\n\nAtoms\n\n"%(natoms,natoms-1, natoms-2,box_side+1,box_side+1,box_side+1,mass_atom,mass_atom))
